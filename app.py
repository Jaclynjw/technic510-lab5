...
def translate_to_english(user_input):
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": f"Translate the following text into natural spoken English: '{user_input}'"},
        ],
        temperature=0.5,
        top_p=1.0
    )
...
def conversation_reply(user_input):
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "Act as a native English speaker in a casual conversation. Respond naturally in English."},
